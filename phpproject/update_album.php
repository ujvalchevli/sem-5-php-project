...
$query = "SELECT * FROM `tbl_album` WHERE `Aid` = $id";
                ...
                <div class="form-group">
                    ...
                    <div class="file-upload">
                        <input type="file" id="albumImage" name="albumImage" class="file-upload-input" accept="image/*"
                            value="<?= $row2['Apic'] ?>">
                        <img src="<?= $row2['Apic'] ?>" height="120" width="120">
                    ...
                    <div class="form-group">
                        ...
                        <select id="categorySelect" name="categorySelect" required>
                            ...
                            <?php
                            foreach ($carr as $c): ?>
                                <option value="<?= $c['cat_name'] ?>" <?= ($row2['category'] == $c['cat_name']) ? 'selected' : '' ?>>
                                    <?= $c['cat_name'] ?>
                                </option>
...
if (isset($_POST['btnsumit'])) {
    ...
    if (empty($temp_album_pic)) {
        $album_pic = $row2['Apic'];
    } else {
        move_uploaded_file($temp_album_pic, $album_pic);
    }
    $update_query = "UPDATE `tbl_album` SET `Aname`='$albumName',`Apic`='$album_pic',`category`='$categorySelect' WHERE `Aid`= $id";
    if (mysqli_query($con, $update_query)) {
        echo "<script>alert('Album updated successfully.'); window.location.href='album.php';</script>";
    } else {
        echo "<script>alert('Error updating album: " . mysqli_error($con) . "');</script>";
    }
}
?>